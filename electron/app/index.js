...
var Page = {
  ...
  ncatch() {
    ...
    $('#catch').on('click', function() {
      ...
      Program.callback = function(bookName, bookInfo) {
        ...
        var size = me.getFileSize(curPath);

        $('#chapter').html(`<a href="file://${curPath}" download="${bookName}">${bookName}</a> / 大小:${size} / 字数:${bookInfo.count}`);
        // history
  ...
  getFileSize(path) {
    ...
    var size;
    if((states.size / 1024 / 1024) > 1) {
      size = (states.size / 1024 / 1024).toFixed(2) + 'mb';
    }else{
      size = Math.round(states.size / 1024) + 'kb';
    }
    return size;
  }
}
